...
>((acc, provider) => {
    ...
    switch (provider) {
        ...
        case "google":
            ...
            if (env("CALLBACK_URL")) config.redirectUri = env("CALLBACK_URL");
...
export const auth = betterAuth({
    ...
    plugins: [
        ...
        customSession(async ({ user, session }) => {
            ...
            let currentImage = dbUser?.image || user.image;
    ...
    advanced: {
        // Share auth cookie across ports (frontend :3000, backend :8000)
        crossSubDomainCookies: {
            enabled: true,
            domain: process.env.BETTER_AUTH_COOKIE_DOMAIN || "localhost",
        },
